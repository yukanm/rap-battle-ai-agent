...
export interface GenerateLyricRequest {
  theme: string
  bars?: number  // 小節数（オプショナル）
  previousLyrics?: string[]  // これまでのリリック
  opponentLatestLyric?: string  // 相手の最新リリック（アンサー用）
  style: string
  model: 'gemini-flash' | 'gemini-pro'
  maxTokens?: number
  roundNumber?: number  // 現在のバース
  totalRounds?: number  // 全バース数
}
